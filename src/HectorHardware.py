...
class HectorHardware(HectorAPI):
    ...
    def __init__(self, cfg=None):
        ...
        if not DevEnvironment:
            ...
            # setup scale (HX711)
            hx1 = self.config["hx711"]["CLK"]
            hx2 = self.config["hx711"]["DAT"]
            hxref = self.config["hx711"]["ref"]
        if not DevEnvironment:
        ...
        self.valvePositions = self.config["pca9685"]["valvepositions"]
        self.fingerChannel = self.config["pca9685"]["fingerchannel"]
        self.fingerPositions = self.config["pca9685"]["fingerpositions"]
        self.lightPin = self.config["pca9685"]["lightpin"]
        self.lightChannel = self.config["pca9685"]["lightpwmchannel"]
        self.lightPositions = self.config["pca9685"]["lightpositions"]
        if not DevEnvironment:
            pcafreq = self.config["pca9685"]["freq"]
            self.pca = Adafruit_PCA9685.PCA9685()
            self.pca.set_pwm_freq(pcafreq)

        # setup arm stepper (A4988)
        self.armEnable = self.config["a4988"]["ENABLE"]
        self.armReset = self.config["a4988"]["RESET"]
        self.armSleep = self.config["a4988"]["SLEEP"]
        self.armStep = self.config["a4988"]["STEP"]
        self.armDir = self.config["a4988"]["DIR"]
        self.armNumSteps = self.config["a4988"]["numSteps"]
        self.simulatedArmPos = self.armNumSteps / 2  # 50%
        print("arm step %d, dir %d" % (self.armStep, self.armDir))
        self.arm = self.config["arm"]["SENSE"]
        if not DevEnvironment:
        ...
        # setup air pump (GPIO)
        self.pump = self.config["pump"]["MOTOR"]
        if not DevEnvironment:
